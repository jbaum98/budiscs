...
OrdersSchema = new SimpleSchema({
    name: {
        ...
        autoValue: function() {
            'use strict';
            if (this.isSet) {return this.value.trimplus().capitalize();}
        },
        custom: function () {
            'use strict';
            Meteor.call('checkDuplicateOrder', this.value, this.field('bunk'), this.field('sale_id'), function(error, result) {
               if (!result) {
                   Orders.SimpleSchema().addInvalidKeys([{name: 'duplicate'},{bunk:'duplicate'}])
               } 
            });
        }
    },
    ...
    discs: {
        ...
        custom: function() {
            ...
            else if (this.value.length !== Sales.findOne(this.field('sale_id'), {fields: {colors: 1}}).colors.length) {
